    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            for row in rows:
                ...
                # Increment counter and bulk insert every 100 rows
                row_count += 1
                if row_count % 100 == 0:
                    insert_location_data_bulk(connection, data_to_insert)
                    data_to_insert = []  # Clear the list after bulk insert
                    print(f"Processed and inserted {row_count} rows... committing changes.")
                    connection.commit()

            # Final bulk insert and commit after processing all rows
            if data_to_insert:
                insert_location_data_bulk(connection, data_to_insert)
                connection.commit()
